...
def model_result(tmp_path_factory):
    ...
    output_dir = tmp_path_factory.mktemp("iw_yolo_model_output")
    ...
    return run_iw_yolo_on_one_record(
        detector=detector,
...
def test_iw_yolo_model_runs(model_result):
    assert model_result["cycle"] == "001"
    assert model_result["pass"] == "161"
    assert model_result["filter_type"] == IW_YOLO_FILTER_TYPE
    assert IW_YOLO_FILTER_TYPE == "rolling"
    assert isinstance(model_result["detection_count"], int)

    assert Path(model_result["mask_netcdf"]).exists()
    assert Path(model_result["summary_text"]).exists()
    assert Path(model_result["boxed_preview"]).exists()


def test_model_output_becomes_geojson(model_result):
    features = mask_netcdf_to_features(model_result)
